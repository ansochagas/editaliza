...
// Obter preview do status do cronograma (dados reais do usuário)
app.get('/plans/:planId/schedule_preview', 
    authenticateToken,
    validators.numericId('planId'),
    handleValidationErrors,
    async (req, res) => {
        try {
            ...
            // Buscar todos os tópicos do plano
            ...
            // Calcular cobertura real (tópicos agendados vs total)
            ...
            // Calcular estatísticas de revisão (3 ciclos por tópico: 7D, 14D, 28D)
            const revisionCycles = completedTopics > 0 ? Math.round(revisionSessions / completedTopics) : 0;
            const expectedRevisionsPerTopic = 3; // 3 ciclos: 7 dias, 14 dias, 28 dias
            ...
            console.error('Erro detalhado ao buscar preview do cronograma:', err);
            console.error('Stack trace:', err.stack);
            res.status(500).json({ "error": "Erro ao buscar preview do cronograma", "details": err.message });
        }
});
